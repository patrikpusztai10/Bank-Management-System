#include "header.h"
int validate_username(char s[50])
{
...
void create_account()
{
    ...
    scanf("%50s",&name);
    while(validate_username(name)!=1)
    {
        printf("INVALID USERNAME {%c} IS NOT A LETTER.\nEnter your name:",validate_username(name));
        scanf("%49s",&name);
    }
    printf("2.Enter your surname:");
    scanf("%49s",&surname);
    while(validate_username(surname)!=1)
    {
        printf("INVALID USERNAME {%c} IS NOT A LETTER.\nEnter your name:",validate_username(surname));
        scanf("%s",&surname);
    }
